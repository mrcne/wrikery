...
// errNoDataCenter is returned when every host in the list answered 300, so the token belongs to
// none of the data centers this build knows about.
...
// hc is optional, a nil value leaves the client on its own default transport, tests pass one that
// routes to a fake server instead of the real Wrike hosts.
// A wrong host data center answers 300 to every request, see wrike.APIError.IsWrongHost, so that
// error moves on to the next host. Any other error, including a 401, comes back at once: it says
// something about the token, not about the host, and trying another host would not change it.
